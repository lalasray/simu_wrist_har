...
if pose_type == "pose":
    ...
    if pose_encoder_type == "attention":
        ...
        print("ToDo")
         ...
         class PoseEncoder(nn.Module):
            def __init__(self, input_dim=(30,25600), embedding_dim=512):
                super(PoseEncoder, self).__init__()
                self.conv1 = nn.Conv1d(in_channels=input_dim[0], out_channels=32, kernel_size=4, padding=1)
                self.conv2 = nn.Conv1d(in_channels=32, out_channels=128, kernel_size=4, padding=1)
        ...
        class PoseEncoder(nn.Module):
            def __init__(self, input_dim= 30*1024*25, embedding_dim = 512):
                super(PoseEncoder, self).__init__()
                self.encoder = nn.Sequential(
                    nn.Linear(input_dim, embedding_dim*2),
                    nn.ReLU(),
