...
class InfonceLoss(nn.Module):
    ...
    def forward(self, query, positive_key, negative_keys=None):
        return inf_nce(query, positive_key, negative_keys,
                        temperature=self.temperature,
                        reduction=self.reduction,
                        negative_mode=self.negative_mode)


def inf_nce(query, positive_key, negative_keys=None, temperature=0.7, reduction='mean', negative_mode='unpaired'):
...
class newInfonceLoss(nn.Module):
    def __init__(self, temperature=0.07):
        super(newInfonceLoss, self).__init__()
        # Initialize InfoNCE loss from the info_nce library
        self.loss_fn = InfoNCE(temperature=temperature)

    def forward(self, q, k):
        # Compute InfoNCE loss using in-batch negatives (each sample compared to all others in the batch)
